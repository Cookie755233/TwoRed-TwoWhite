...
![Turbojet Engine Diagram](/images/turbojetengine.png "Turbojet Engine Diagram")

#### Schematic
Inlet air
→ Drives the Turbine 
→ Drives the Compressor/Gear box
→ Then turn the propeller
→ Combustion Chamber contains fuel for combustion (Ignition)
→ Expanding air drives a turbine (Power Extraction)
→ → Turbine connected by shaft to the compressor
→ Exhaust from the engine (Thrust)
